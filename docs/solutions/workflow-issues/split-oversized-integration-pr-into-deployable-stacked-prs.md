...
`runbooks/guarded-pr-merge.sh`는 exact repo/PR 승인과 live metadata packet을 확인하지만 `MERGEABLE/CLEAN` 자체를 조건문으로 강제하지는 않는다. 따라서 직전 단계의 live gate를 별도로 통과한 뒤 현재 turn의 approval packet대로 실행한다.
...
`--delete-branch`는 생략한다. 명령 완료 후 PR의 `MERGED`와 `mergedAt`을 확인한 뒤에만 해당 Linear issue를 `Done`으로 전환한다. 이어서 새 `main`을 기준으로 다음 direct child에 같은 절차를 반복한다.
...
- **Linear `Done`은 merge 결과다.** PR 생성, reviewer pass, merge order 합의, `CLEAN` 확인은 완료 상태가 아니다.
...
- `runbooks/guarded-pr-merge.sh` — exact repo/PR 승인과 merge metadata packet을 확인하는 wrapper.
