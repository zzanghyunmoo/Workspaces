...
- Project actions: Gradle wrapper와 managed `dotnet`만 사용해 build/test/run/watch를
  실행하고, ASP.NET launch profile은 loopback URL만 허용한다. Java·Kotlin 및 .NET
  application/test debugging은 실제 breakpoint, stack, scope, known variable,
  continue/step/terminate 결과로 판정한다.
...
- Apple Silicon Lima `mds` actual target:
  ...
  - apply receipt `complete: true`; 이어서 실행한 repeat apply의 25개 outcome 전부
    `noop: true`
  - doctor `ready: true`; expected 23개 capability 전부 pass
  ...
  - DAP: Java, Kotlin, .NET app/test 모두 breakpoint·source line·stack·scope·known
    variable·continue·step-in·step-over·terminate 통과
