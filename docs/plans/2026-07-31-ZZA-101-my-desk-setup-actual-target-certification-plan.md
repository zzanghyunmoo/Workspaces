...
경계를 포함한 `all` 대신 target별로 무인 수렴 가능한 네 representative
profile을 사용한다. 첫 release의 Linux archive는 embedded checksum과 정확히
...
- R2. `certification-macos-host`, `certification-windows-host`,
  ...
  target에서 static blocker 없이 해석된다. Workflow의 target→profile
  allowlist가 host·guest의 대표 CLI, toolchain, editor, agent, container와
  guest-lifecycle capability를 합쳐 검증한다.
...
- R6. macOS host, Windows host, WSL guest와 Lima guest를 각각 dedicated
  repository-level runner와 exact custom label로 인증한다.
- R7. 각 target은 read-only plan, exact-digest first apply, 같은 digest의
  repeat all-no-op, doctor와 bounded functional probes를 통과한다.
- R8. Guest evidence는 host committed ownership record, live root-owned image
  marker와 service-inherited creation nonce가 runtime에서 일치해야 한다. Raw
  nonce는 argv, log, receipt와 uploaded artifact에 나타나지 않는다.
...
- R11. 같은 release commit과 immutable certification cohort에 target kind별
  성공 artifact가 정확히 하나이고, manifest capture 완료 시각이 promotion
  시점 기준 24시간 이내여야 한다. 이전 cohort는 삭제하지 않고 promotion
  입력에서 제외한다.
- R12. `v0.1.0` tag는 네 `verified` evidence가 모두 성공한 뒤 같은 commit에
  만들며 생성 뒤 이동하거나 삭제하지 않는다.
- R13. Release workflow는 deterministic assets와 promotion report를
  재검증한 뒤 GitHub Release를 게시한다.
...
- R15. Actual workflow는 `verified` bundle만 업로드하고 promotion은
  `blocked`를 허용하지 않는다. 업로드 직전에 exact file set, checksum과
  credential/nonce/개인 경로 leak scan을 독립 gate로 통과한다.
...
- F3. Host/guest preparation — host plan을 먼저 확인한 뒤 제품 소유 guest를
  만들고 local exact archive로 같은 CLI/catalog revision을 seed한다.
...
- AE1. Xcode가 없는 macOS에서 일반 `--all`은 계속 정직하게
  `action-required`다. `--profile certification`은 static blocker 없이
  실행된다.
...
- AE8. Guest nonce가 없거나 marker와 다르면 capture 전에 실패한다. 실제
  nonce 값은 argv, log, plan과 evidence에 출력되지 않는다.
- AE9. Tag workflow는 exact commit의 네 artifact와 release checksums를
  검증한 뒤에만 release asset을 게시한다.
- AE10. 한 cohort가 24시간을 넘기면 같은 commit에서 새 cohort를 만들 수
  있다. Downloader는 선택된 cohort 안의 exactly-one만 승인하고 이전
  cohort를 duplicate로 오인하지 않는다.
...
- `v0.1.0` GitHub Release와 `mds.release-promotion/v1` report가 같은 commit을
  가리킨다.
...
- KTD2 — Target 조건을 profile schema에 새로 넣지 않고 네 target-specific
  ...
  fail-closed mapping한다. Release gate는 이 profile들의 합으로 representative
  surface를 증명하고, 일반 `all`/`owner` 의미는 기존 contract test로 보존한다.
...
- KTD6 — Runner는 target별 전용 OS account와 custom label 하나만 가진다.
  GitHub 등록·token 입력, one-job `--ephemeral` lifecycle과 guest nonce
  drop-in은 사용자 작업이다. Runner credential은 workload에 노출하지 않고
  job 뒤 runner config/work directory를 unregister·scrub한다.
- KTD7 — 각 시도는 strict 형식의 immutable cohort ID
  `cert-<UTC YYYYMMDDThhmmssZ>-<commit8>`를 공유한다. Exactly-one과 24시간
  freshness는 선택된 commit+cohort 내부에서 검증하며 freshness는 evidence
  manifest의 capture 완료 시각을 기준으로 한다.
- KTD8 — Certification에서 product defect가 나오면 우회하지 않는다. Fix
  commit을 review한 뒤 새 commit에서 네 target 전체를 다시 시작한다.
- KTD9 — Runtime은 raw nonce를 최소 권한 inherited environment에서 직접
  읽고 ownership record와 marker를 memory에서 비교한다. Plan/evidence에는
  `SHA-256("mds.guest-creation-nonce/v1\0" || decoded_nonce)` commitment만
  기록한다. CLI argv, subprocess output와 error payload에는 raw 값을 넣지
  않는다.
- KTD10 — Actual upload와 promotion은 `verified` 전용이다. Capture 실패
  bundle은 runner local에만 보존한다. 업로드 전 exact-file-set/checksum/leak
  scan을 통과한 bundle만 cohort 이름으로 게시한다.
- KTD11 — `v*` tag는 update/delete가 금지된다. Tag workflow는 같은 tag와
  commit에 idempotent한 draft release를 구성하고 assets와 promotion report를
  검증한 뒤 publish한다. 실패하면 tag를 이동·삭제하지 않고 같은 identity로
  재시도하며, byte mismatch면 version을 미게시 상태로 폐기한다.
...
flowchart LR
  ...
  E3 --> PROMOTE
  E4 --> PROMOTE
  PROMOTE --> TAG[v0.1.0 tag]
  TAG --> RELEASE[Promotion report and GitHub Release]
```

### System-Wide Impact

- Catalog — 네 profile은 resolver와 catalog revision에 포함된다.
- CLI/adapter — local seed source가 host guest handoff에 추가되지만 plan과
  receipt schema는 바꾸지 않는다.
- Transport — WSL/Lima는 embedded script를 command payload로 실행하고
  archive snapshot만 stdin으로 받는 같은 bounded byte-stream 계약을 쓴다.
- Evidence — raw nonce field를 commitment로 바꾸고 cohort identity를
...
- Runner registration과 nonce drop-in은 user-owned auth/privilege
  dependency다. Agent가 token이나 nonce를 요청·저장하지 않는다.
...
- Files:
  ...
  - `projects/my-desk-setup/tests/contract/`
  - `projects/my-desk-setup/tests/target-evidence/README.md`
- Approach: Target-specific closed profile 네 개와 workflow의 exact
  target→profile allowlist를 추가한다. Capture와 expected-plan input 설명을
  같은 mapping으로 고정하고 verifier를 `--require-verified`로 실행한다.
- Test scenarios:
  ...
  - Profile의 capability coverage, unknown ID와 schema drift가 fail closed한다.
...
- Approach: Apply-only `--guest-bootstrap-archive`를 host guest-lifecycle
  ...
  script는 command payload로 분리한다. Raw nonce는 inherited environment에서
  읽어 runtime에서만 비교하고 persisted target facts에는 KTD9 commitment를
  기록한다.
- Test scenarios:
  ...
  - Raw nonce가 plan/evidence JSON, argv, output와 error payload에 없고
    commitment 변조는 verifier가 거부한다.
...
- Goal: U1–U2를 reviewed merge commit으로 만들고 GitHub 보호 경계, retry
  cohort와 deterministic pre-tag artifact를 준비한다.
- Requirements: R1, R9–R12, R15
- Flows: F1–F2
- Acceptance: AE7–AE8
- Files:
  - `docs/works/2026-07-31-ZZA-101-my-desk-setup-actual-target-certification-work.md`
  - `projects/my-desk-setup/`의 U1–U2 diff
...
- Approach: Latest head에 code/doc review marker와 CI를 만든 뒤 guarded
  merge한다. 새 main에서 branch protection, `v*` tag ruleset과 secret-free
  environment를 구성한다. Strict cohort input을 manifest와 artifact name에
  결합하고 selected cohort 내부 exactly-one을 검증한다. 사용자가 target별
  one-job ephemeral runner를 등록한 뒤 pre-tag `release-dist`를 만든다.
  Capture workflow는 raw nonce를 argv로 확장하지 않고 inherited environment
  그대로 CLI에 전달한다. Verified bundle은 exact-file-set/checksum과
  credential/nonce/개인-path scan이 성공한 경우에만 업로드한다.
- Test scenarios:
  - Unprotected ref에서는 Actual target이 skip된다.
  - Wrong target/label pair와 dirty runner workspace는 실패한다.
  - Guest nonce missing과 host nonce presence는 capture 전에 raw 값을
    출력하지 않고 실패한다.
  - Invalid cohort, cross-cohort mix, selected-cohort duplicate와 manifest
    capture 완료 시각 기준 stale evidence를 거부한다.
...
- Verification: 새 merge commit의 CI가 green이다. GitHub API read가 protected
  ...
  두 번 빌드한 `release-dist`가 byte-identical이다.
- Dependencies: U1, U2
...
- Approach: read-only inventory와 plan/digest를 먼저 제시한다. 사용자가
  mutation을 확인한 뒤 host apply와 local seed로 `mds` Lima guest를
  준비한다. 하나의 fresh cohort ID를 발급하고 host/guest ephemeral runner와
...
- Test scenarios:
  ...
  - Lima runner nonce가 ownership record와 live marker에 exact match한다.
...
- Requirements: R4–R11
- Flows: F3–F4
- Acceptance: AE2–AE3, AE5–AE8
- Files:
  - Target-local state와 runner directories
  - GitHub Actions run과 target evidence artifacts
- Approach: Windows host plan과 WSL prerequisite를 먼저 확인한다. 사용자가
  reboot, first-user와 ephemeral runner 등록을 완료한다. U4와 같은 cohort의
  local exact seed, ownership ACL, root marker와 nonce를 검증한 뒤 두
  dispatch를 실행한다.
- Test scenarios:
  - Windows ownership record ACL은 owner와 허용 SID만 가진다.
  - WSL distro, image, marker와 nonce가 exact match한다.
  - Windows host runner에는 guest nonce가 없다.
  - Windows bash, Git과 runner service prerequisite가 충족된다.
  - First apply, repeat all-no-op와 doctor/probe가 성공한다.
...
- Goal: 네 fresh artifact를 같은 commit의 최초 release로 승격하고 추적
  문서를 닫는다.
- Requirements: R11–R14
- Flows: F5
- Acceptance: AE6–AE7, AE9
- Files:
  - GitHub tag, release와 promotion artifact
...
- Approach: 선택한 cohort를 고정한 local promotion rehearsal로 duplicate,
  stale, non-verified와 identity mismatch를 먼저 거부한다. `v*` ruleset을
  재확인하고 tag를 같은 commit에 만든다. Workflow는 같은 tag/commit의 draft
  release에 deterministic assets를 idempotently stage하고 promotion report와
  published checksums를 검증한 뒤 publish한다. 실패 시 tag를 이동·삭제하지
  않고 같은 identity로 재시도한다. Release 성공 뒤 Notion을 먼저
  closeout한다. Local KB/work와 root gitlink를 동기화한 뒤 Linear를
  `Done`으로 바꾼다.
- Test scenarios:
  ...
  - Published checksums와 promotion report를 다시 검증한다.
...
- GitHub API read로 branch protection, environment와 runner labels를
  확인한다.
- 네 target plan의 CLI revision, catalog revision, digest와 binary SHA-256을
  release manifest와 대조한다.
- Actual evidence verifier를 `--require-verified`로 통과시킨다.
- Upload 직전 exact-file-set/checksum과 credential/raw nonce/개인 절대 경로
  scan을 통과시킨다.
- Promotion rehearsal과 tag workflow에서 selected cohort의 exactly-four,
  manifest capture 완료 시각 기준 freshness 24h와 deterministic checksum을
  재검증한다.
- Tag 생성 직전과 release publish 뒤 API로 `v0.1.0` tag가 approved commit을
  가리키고 update/delete protection이 활성인지 재확인한다.
- CLI/automation 변경에는 browser UI가 없으므로 LFG browser test는
  `not applicable`로 work evidence에 기록한다.

## Definition of Done

- U1–U2 code가 ZZA-101 PR로 review와 CI를 통과해 merge됐다.
...
- `v0.1.0` GitHub Release, checksums와 verified promotion report가 게시됐다.
...
- U1: 각 target fixture에서 대응 certification profile blocker가 0개고 일반
  `all` truthfulness와 verified-only workflow가 회귀하지 않는다.
- U2: exact local seed가 URL path와 같은 identity로 수렴하며 unsafe input,
  TOCTOU와 raw nonce/path leakage test가 통과한다.
- U3: fix merge commit, protected branch/tag control plane, cohort, 네
  ephemeral runner와 deterministic pre-tag artifact가 준비됐다.
...
- U5: Windows와 WSL artifact가 selected cohort에서 exactly one, fresh,
  `verified`다.
- U6: `v0.1.0` release와 모든 closeout artifact가 같은 identity를 가리킨다.
