...
Programming Fundamentals Branch Chain은 C#을 처음 배우는 사용자가 매 장마다 `기본 개념 학습 → 작은 예제 → Sokoban을 만들면서 같은 개념 복습/적용`을 반복하는 36장 장기 학습 경로다. 범위는 `왜 C#인가`에서 시작해 개발환경 설정, 변수와 타입, 제어문, 반복문, 메서드, 문자열/배열, 객체 지향 프로그래밍, 컬렉션, nullable, 예외, 테스트, record, interface, generic, LINQ, file I/O, JSON, async, logging, configuration까지 포함한다.
...
- R3. 기초 문법 챕터는 변수, 타입, 문자열, 숫자, 콘솔 입출력, 표현식을 먼저 작은 예제로 확인한 뒤 Sokoban 상태나 규칙에 적용하게 해야 한다.
...
- D2. Chapter docs cover why C#, OS-specific development environment setup, variables/types, control flow, methods/arrays, OOP, collections, nullable, exceptions, testing, records, interfaces, generics, LINQ, file I/O, JSON, async, debugging, logging, configuration, refactoring, and capstone readiness through the repeated concept-to-Sokoban application loop.
