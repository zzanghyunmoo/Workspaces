...
- **KTD5 — Canonical digest:** sorted canonical JSON의 SHA-256에
  catalog/lock revision, target facts, selection, dependency expansion,
  requested/resolved version, observed preimage, privilege/manual preflight와
  ordered actions를 결합한다.
- **KTD6 — Exact preview/apply:** `plan`은 stdout/JSON에 digest를 출력할
  뿐 state를 쓰지 않는다. `apply`와 `update`는 exact digest를 요구하고 모든
  preimage를 첫 mutation 전에 재검증하며 mismatch는 `stale-plan`로 mutation
  0을 보장한다.
...
- **KTD13 — Explicit update:** normal apply는 latest metadata를 조회하지
  않는다. `update`만 candidate를 exact version/provenance와 lock diff로
  만들며 lock write와 target mutation은 같은 digest에 묶되 Git commit은
  사용자 소유다.
...
- **KTD15 — Stable automation contract:** interactive chooser와 flags는
  같은 Selection을 만들고 모든 명령은 versioned closed JSON envelope, stable
  status와 exit code를 제공한다.
...
sequenceDiagram
  ...
  U->>H: plan --target host,guest --profile all
  H->>H: observe host + resolve host plan
  H->>T: run exact CLI/catalog revision
  T->>G: observe Ubuntu guest + resolve guest plan
  G-->>H: guest plan + digest + blockers
  H-->>U: aggregate read-only plan
  U->>H: apply --plan-digest ...
  H->>H: recompute + reject stale preimage
  H->>T: apply exact guest digest
  T->>G: reconcile dependency DAG
  G->>S: atomic checkpoints + receipts
  G-->>H: target outcome
  H-->>U: versioned aggregate result
```
...
├── cmd/mds/main.go
├── internal/
│   ├── adapters/{host,guest,packages,components}/
│   ├── catalog/
│   ├── cli/
│   ├── doctor/
│   ├── execution/
...
`catalog/locks/versions.lock.yaml`,
...
component의 합집합으로 resolver가 계산한다.
...
### U3. Target discovery, Ubuntu guest provisioning and transport

**Goal:** macOS/Windows/WSL/Lima target을 안전하게 식별하고 Ubuntu 26.04 LTS
guest를 준비하며 같은 revision의 guest-local CLI로 handoff한다.
...
`internal/transport/lima.go`, `internal/guest/provision.go`,
`bootstrap/macos.sh`, `bootstrap/windows.ps1`,
...
- 한 target은 자동 선택하고 복수 target은 explicit ID 없이는 mutation을
  거부한다.
- unreachable guest가 host 독립 plan/apply를 막지 않고 aggregate
  partial-unready를 만든다.
- host와 guest의 CLI/catalog revision 불일치가 stale/blocked로 종료된다.
- systemd unavailable/inactive guest가 Docker 단계 전에 action-required를
  보고한다.
- Covers AE1/AE3. host/guest action과 target facts가 서로 섞이지 않는다.

**Verification:** WSL과 Lima에서 동일한 Ubuntu identity 계약과 guest-local
observation이 성립하고 host process가 guest-owned path를 직접 변경하지 않는다.
...
**Approach:** Cobra command surface는 `plan`, `apply`, `doctor`, `update`만
공개하고 Bubble Tea chooser는 Selection 생성에만 사용한다. planner는
eligibility → dependency expansion → conflict/ownership check → observation →
ordered actions 순서로 immutable plan을 만든다. human과 JSON renderer는 같은
application result를 소비한다.
...
**Approach:** digest/catalog/lock/target/preimage를 첫 write 전에 다시 계산하고
single-writer lock을 획득한다. 각 action 직전 preimage, 직후 functional
verification과 journal checkpoint를 수행한다. OS package transaction rollback을
약속하지 않고 action별 truth를 남긴다.
...
action-required로 드러낸다. Xcode는 external/manual component이며 Linux
지원으로 승격하지 않는다.
...
mise lock은 language/build versions를 소유한다. NvChad-derived config는 reviewed
commit을 pin하고 pre-existing `~/.config/nvim`을 overwrite하지 않는다. Claude
Code/OpenCode/Codex auto-update는 normal apply에서 비활성화하고 update만
움직인다. CLI와 agent verification은 login 없이 `--version`, help 또는 local
bounded smoke만 실행한다. Docker는 systemd-active guest에서 guest-local
daemon/socket만 소유하고 Docker Desktop 연동은 탐지 시 conflict로 처리한다.
